% FILE:     EmitX86.m
% PURPOSE:  Emit executable code (target dependent)
%           addons for Intel 64 bit in Emit.m
%
% USAGE:   emit = EmitX86(p, syms, errCt, exes, frms)
%          emit = EmitX86(p, syms, errCt, exes, frms, flags)
...
function obj = EmitX86(...
  ...
  HDW = computer;                                % underlying hardware
  EM = findFlag('-emulate', allFlags) ...        % user requests emulation
    || findFlag('-exeTrace', allFlags)...
    || findFlag('-interactive', allFlags);
  
  switch HDW                                     % may force emulation
    case {'PCWIN', 'GLNX6', 'MACI', 'GLNXA64'}   % OK to execute X86
    otherwise                                    % cannot execute
      EM = true;                                 % must emulate
  end
...
% ------------------- control generators -------------------
  ...
  function prolog()                              % calling seq. in
    ...
    if EM || any(strcmp(HDW, {'PCWIN', 'GLNX86', 'MACI'}))
      asm.prolog();
    elseif strcmp(HDW, 'GLNXA64')
      asm.prologq();
    else
      emitErr(['unexpected platform ' HDW]);     % firewall
    end      
    if TE; TR('prolog', -1); end
  end
  ...
  function res = realRand()
    ...
    if EM || any(strcmp(HDW, {'PCWIN', 'GLNX86', 'MACI'}))
      if int32(bitshift(addr, -32)) ~= 0         % maybe running on A64
        addr = 112;                              % magic number for emulator
      end
      asm.movRC(asm.EAX, addr, '# addr of rand');  % C code location
      asm.callRi(asm.EAX, '# call runtime');     % go do it
    elseif strcmp(HDW, 'GLNXA64')
      asm.pushR(asm.RSI);                        % save X frame
      asm.movRCq(asm.RAX, addr, '# addr of rand');  % C code location
      asm.callRiq(asm.RAX, '# call runtime');    % go do it
      asm.popR(asm.RSI);                         % restore x frame
    else
      emitErr(['unexpected platform ' HDW]);     % firewall
    end      
  ...
  function intDiv(r, left, right, err)
    ...
    elseif strcmp(computer, 'GLNXA64')
      asm.pushR(asm.RSI, '    # frame pointer'); % save frame pointer
      asm.pushR(asm.RBX, '    # Nikolay says');  % save RBX
      asm.movRR(asm.ESI, r2, '# divisor');                     
      asm.movRR(asm.EDI, r1, '# dividend');
      toA(left);
      toA(right);
      spillAllRegs();                             % call-side save
      asm.movRCq(asm.RAX, addr, ['# addr of ' dop '']);  % C code location
      asm.callRiq(asm.RAX, '   # call runtime');  % go do it
      asm.popR(asm.RBX, '     # Nikolay says');   % restore RBX
      asm.popR(asm.RSI, '     # frame pointer');  % recover frame pointer
      %asm.addRCq(asm.RSP, 8, '# destroy frame');  % destroy frame
    else
      emitErr(['platform ' HDW ' NYI']);         % firewall
    end
