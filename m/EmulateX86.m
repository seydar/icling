...
function obj = EmulateX86()                       % ctor
  ...
  function [rc, outframe] = trace(x86code, len, inframe, st, mode)
    ...
    rc = 0;                                      % normal exit
    fprintf('Start x86 emulation: len=%d frame=%d words\n', ...
             len, numel(inframe));
    while ~done    
  ...
  function [in, rg] = singleStep(newstep)
    h = sprintf('%4d/%40x: ', newstep, pc);      % standard header
      ...
      switch op                                  % sorted for sanity
        ...
        case hx2('8b')
          ...
          elseif m == 1                          % movRP
            ...
            ofs = opd1/4;
            if ofs == 2
              iregs(r2+1) = 0;                   % fakeXXX calling sequence
            end
        ...
        case hx2('ff')
          ...
          if iregs(r1+1) == getCfun('rand')
            iregs(R.EAX+1) = f2i(single(rand)); % wrong rand but who can tell?
