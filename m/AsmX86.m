...
function obj = AsmX86(subFlags, allFlags)        %#ok ctor

  if nargin < 2; allFlags = {}; end              %#ok
  ...
  function prolog()
    ...
    pushA('        # callee save everything');   % callee save iregs
  end

  function epilog()
    popA('      # callee restore everything');
...
% ---------------- double/single instructions -----------------
  ...
  function fsin(msg)                             % sin
    if nargin < 1; msg = ''; end
    if TA; T(['fsin ', msg]); end
    realPrefix(hx4('fed9'));
  end
  function fcos(msg)                             % cos
    if nargin < 1; msg = ''; end
    if TA; T(['fcos ', msg]); end
    realPrefix(hx4('ffd9'));
  end
